...
// Flexible helper to register a user (accepts extra fields)
async function registerUser(email: string, password: string, extra: Record<string, any> = {}) {
  return request(app)
    .post('/api/auth/register')
    .send({ email, password, ...extra });
}

// Flexible helper to login a user (accepts extra fields)
async function loginUser(email: string, password: string, extra: Record<string, any> = {}) {
  return request(app)
    .post('/api/auth/login')
    .send({ email, password, ...extra });
}
...
async function getValidToken(email: string, password: string) {
...
describe('Auth API', () => {
  ...
  describe('/api/auth/me', () => {
    ...
    beforeEach(async () => {
      ...
      token = await getValidToken('meuser@example.com', 'Test1234!');
    });
